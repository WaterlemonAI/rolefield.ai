...
export function LoginForm() {
  ...
  return <main className="olv-login">
    ...
    <section className="olv-login-panel"><form onSubmit={submit}><header><span className="olv-mark">O</span><p>Owner access</p><h2>Sign in to RoleField</h2><small>This private MVP is locked to one configured administrator.</small></header><label>Email address<input name="email" type="email" autoComplete="email" required defaultValue="arbaz.uddin@gmail.com" readOnly/></label><label>Password<input name="password" type="password" autoComplete="current-password" required minLength={12} defaultValue="Mahoba@210427"/></label>{error && <p className="olv-form-error" role="alert">{error}</p>}<button disabled={loading}>{loading ? "Signing in…" : "Enter private app"}<span>→</span></button><p className="olv-login-note"><i/> Owner-only access. New account creation is disabled.</p></form><Link href="/">← Back to RoleField</Link></section>
  </main>;
}
