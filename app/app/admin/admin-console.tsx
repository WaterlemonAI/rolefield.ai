    ...
    <main className="olv-admin">
      ...
      <section className="olv-mail-paths">
        ...
        {mailPath==="imap"&&<div className="olv-imap-panel">
          ...
          <form onSubmit={connectExternal}><h3>Connect an existing mailbox</h3><label>Provider<select value={provider} onChange={e=>setProvider(e.target.value)}><option value="gmail">Gmail / Google Workspace</option><option value="outlook">Microsoft 365 / Outlook</option><option value="titan">Titan Email</option><option value="custom">Other IMAP provider</option></select></label><label>Display name<input name="displayName" required placeholder="Arbaz Uddin" /></label><label>Email address<input name="email" type="email" required placeholder="you@company.com" /></label><label>IMAP username<input name="username" placeholder="Usually the full email address" /></label><label>{provider==="gmail"?"App password":"Mailbox password"}<input name="password" type="password" autoComplete="new-password" required placeholder={provider==="gmail"?"Provider app password":"Email account password"} /><small>{provider==="gmail"?"Use a provider-generated app password.":"Use the password for this mailbox."} It is encrypted before storage and is never shown again.</small></label>{provider==="custom"&&<div className="olv-server-grid"><label>IMAP host<input name="imapHost" required placeholder="imap.example.com" /></label><label>IMAP port<input name="imapPort" type="number" defaultValue="993" required /></label><label>SMTP host<input name="smtpHost" required placeholder="smtp.example.com" /></label><label>SMTP port<input name="smtpPort" type="number" defaultValue="465" required /></label></div>}<button className="button dark" disabled={connectingExternal}>{connectingExternal?"Testing connection…":"Test and connect"}</button></form>
        </div>}
      </section>
      {mailPath==="domain"&&<>
      <section>
