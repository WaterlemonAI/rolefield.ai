...
export async function POST(request: Request) {
  try {
    ...
    await db.prepare("CREATE INDEX IF NOT EXISTS idx_demo_bookings_slot ON demo_bookings(demo_date, demo_time)").run();
    await db.prepare(`INSERT INTO demo_bookings
      (booking_id, name, email, company, phone, language, use_case, attendees, notes, demo_date, demo_time, timezone, status, created_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'Asia/Dubai', 'confirmed', ?)`)
      .bind(
