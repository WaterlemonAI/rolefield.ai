...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const translate = (node: Node) => {
      ...
      while ((textNode = walker.nextNode())) {
        ...
        if (translated) textNode.nodeValue = raw.replace(value, translated);
      }
    };
    translate(root);
  ...
  return <main ref={pageRef} lang={locale} dir={locale === "ar" ? "rtl" : "ltr"} className={locale === "ar" ? "rtl" : "ltr"}>
    ...
    <section className="agent-section section" id="agents">
      ...
      {conversation && <div className="integration-note" role="status"><div><i/><b>{locale === "ar" ? `${agents.find(a=>a.id===conversation)?.arabic} جاهز.` : `${agents.find(a=>a.id===conversation)?.name} is ready.`}</b><span>The live voice connection plugs in here. Book a working demo to speak with this agent using your own use case.</span></div><button className="button dark compact" onClick={openScheduler}>Book live session</button></div>}
    </section>
    ...
    <section className="final-cta technical-grid"><div><p className="eyebrow"><i/> READY FOR A REAL CONVERSATION?</p><h2>Your next customer conversation can be handled by <em>RoleField.</em></h2><div><a href="#agents" className="button teal">Talk to an agent ↘</a><button className="button dark" onClick={openScheduler}>Book a GCC demo</button></div></div><div className="cta-agents"><Image className="noura" src="/agents/noura.png" alt="Noura, RoleField AI agent" width={1224} height={1285}/><Image className="hamdan" src="/agents/hamdan.png" alt="Hamdan, RoleField AI agent" width={1312} height={1199}/><Image className="adam" src="/agents/adam.png" alt="Adam, RoleField AI agent" width={1312} height={1199}/></div></section>
