...
export interface MailProvider { createDomainIdentity(domain:string):Promise<DnsRecord[]>; checkDomainIdentity(domain:string):Promise<{identity:boolean;dkim:boolean}>; sendRaw(raw:Uint8Array):Promise<string>; }
export class SesMailProvider implements MailProvider {
  ...
  async checkDomainIdentity(domain:string){const {ses}=clients();const result=await ses.send(new GetEmailIdentityCommand({EmailIdentity:domain}));return {identity:result.VerifiedForSendingStatus===true,dkim:result.DkimAttributes?.Status==="SUCCESS"};}
