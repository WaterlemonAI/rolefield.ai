...
function imapError(error: unknown) {
  const value=error as {message?:string;response?:string;responseText?:string;responseStatus?:string;serverResponseCode?:string;authenticationFailed?:boolean;code?:string};
  const details = [value.responseText, value.response, value.serverResponseCode, value.responseStatus, value.code]
    .filter((item): item is string => typeof item === "string" && item.trim().length > 0);
  if (value.authenticationFailed) details.unshift("The IMAP server rejected the supplied login");
  if (value.message && value.message !== "Command failed") details.push(value.message);
  return [...new Set(details)].join(" · ") || "The IMAP server rejected the command without an explanation";
}
export async function testExternalMail(config: ExternalMailConfig) {
  ...
  catch(error) { throw new Error(`IMAP login failed (${config.imapHost}:${config.imapPort}, ${config.imapSecure ? "SSL/TLS" : "STARTTLS"}): ${imapError(error)}`); }
  try { await imap.mailboxOpen("INBOX", { readOnly: true }); }
  catch(error) { throw new Error(`IMAP inbox access failed after login: ${imapError(error)}`); }
