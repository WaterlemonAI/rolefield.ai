...
export async function requireAdmin(){const p=await requirePrincipal();if(p.orgRole!=="ADMIN")redirect("/app");return p;}
export async function requireModule(module:AppModule){const p=await requirePrincipal();if(p.orgRole!=="ADMIN"&&!p.modules.includes(module))redirect("/app");return p;}
...
export async function apiRequireAdmin(request:Request){const p=await apiPrincipal(request);if(p.orgRole!=="ADMIN")throw Object.assign(new Error("Administrator access required."),{status:403});return p;}
export async function apiRequireModule(request:Request,module:AppModule){const p=await apiPrincipal(request);if(p.orgRole!=="ADMIN"&&!p.modules.includes(module))throw Object.assign(new Error(`${module} access required.`),{status:403});return p;}
